...
- The API must be running locally using `docker-compose up` or directly from the backend

## Test Files

- `main.js`: Primary test using the Leaky Bucket algorithm with complete user workflow  
- `api-performance.js`: General API performance testing  
- `auth-test.js`: User authentication testing  

## Running Tests

### Primary Leaky Bucket Test
...
After running tests, K6 will display detailed metrics in the terminal, including:
...
## Tuning Parameters and Limitations

- **VUs (Virtual Users)**: Modify to test different concurrency levels  
- **Duration**: Adjust test duration as needed  
- **Thresholds**: Set acceptance thresholds based on your environment and requirements
