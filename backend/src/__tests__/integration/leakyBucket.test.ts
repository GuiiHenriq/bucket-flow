...
describe('Leaky Bucket Integration Tests', () => {
  ...
  beforeAll(() => {
    authToken = jwt.sign({ id: TEST_USER_ID }, JWT_SECRET, { expiresIn: '24h' });
    server = app.listen();
  });
  ...
  const makeAuthenticatedRequest = (method: 'get' | 'post', path: string, data?: any) => {
    const req = request(server)[method](path).set('Authorization', `Bearer ${authToken}`);
  ...
  describe('POST /api/pix/query', () => {
    ...
    it('should process query when tokens are available', async () => {
      ...
      const response = await makeAuthenticatedRequest('post', '/api/pix/query', { key: testKey });
      expect(response.status).toBe(200);
    ...
    it('should maintain token count after query', async () => {
      jest.spyOn(global.Math, 'random').mockReturnValue(0.7);
      await makeAuthenticatedRequest('post', '/api/pix/query', { key: testKey });
      const response = await makeAuthenticatedRequest('get', '/api/tokens');
